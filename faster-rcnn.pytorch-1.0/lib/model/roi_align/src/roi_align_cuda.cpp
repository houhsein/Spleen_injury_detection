...
int roi_align_forward_cuda_3d(at::Tensor features, at::Tensor rois, int pooled_depth,
                           int pooled_height, int pooled_width,
                           float spatial_scale, float spatial_scale_depth, int sample_num,
                           at::Tensor output) {
  CHECK_INPUT(features);
  CHECK_INPUT(rois);
  CHECK_INPUT(output);
  ...
  ROIAlignForwardLaucher3D(features, rois, spatial_scale, spatial_scale_depth, sample_num,
                           num_channels, data_depth, data_height, data_width, num_rois,
                           pooled_depth, pooled_height, pooled_width, output);
...
int roi_align_backward_cuda_3d(at::Tensor top_grad, at::Tensor rois,
  ...
  ROIAlignBackwardLaucher3D(top_grad, rois, spatial_scale, spatial_scale_depth, sample_num,
                          num_channels, data_depth, data_height, data_width, num_rois,
                          pooled_depth, pooled_height, pooled_width, bottom_grad);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("forward3d", &roi_align_forward_cuda_3d, "Roi_Align forward 3D (CUDA)");
  m.def("backward", &roi_align_backward_cuda, "Roi_Align backward (CUDA)");
  m.def("backward3d", &roi_align_backward_cuda_3d, "Roi_Align backward 3D (CUDA)");
}
